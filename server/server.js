...
app.post('/api/train-recommender',  async (req, res) => { /* Full endpoint included below */ });
app.post('/api/recommend-similar-questions', async (req, res) => { /* Full endpoint included below */ });
app.post('/api/predict-difficulty',  async (req, res) => { /* Full endpoint included below */ });
...
app.post('/login', (req, res) => { const { password } = req.body; if (bcrypt.compareSync(password, ADMIN_PASSWORD_HASH)) { res.json({ accessToken: jwt.sign({ user: 'admin' }, JWT_SECRET, { expiresIn: '8h' }) }); } else { res.status(401).json({ success: false, message: 'Incorrect password' }); } });
app.post('/login-dashboard',  (req, res) => { const { password } = req.body; if (bcrypt.compareSync(password, ADMIN_PASSWORD_HASH)) { res.json({ accessToken: jwt.sign({ user: 'dashboard_admin' }, JWT_SECRET, { expiresIn: '8h' }) }); } else { res.status(401).json({ success: false, message: 'Incorrect password' }); } });
